...
- [ ] 1.3 根据基线和维护收益决定是否继续拆 persistence、recovery、coordinator；不以“全部拆完”作为前置条件。
  - **自测标准**：每个实际拆分均有边界测试、Rust/前端测试、格式和 Clippy 证据。

## Phase 1：可观测性验收

- [ ] 2.1 在同一设备、C:/D: 范围、Release 构建和固定数据快照上完成当前 HEAD 三轮完整应用级基线。
  - **自测标准**：归档 operation/scope 指标、发现数、可搜索数、代表性查询和三轮中位数；历史数据仅作背景。
- [ ] 2.2 在真实 Tauri/WebView 中采集索引和查询期间的输入延迟 p95。
...
- [ ] 4.1 补充 active/staging/previous 切换的 operation ID、阶段、路径、目录状态、并发查询、句柄诊断和重试日志。
- [ ] 4.2 在并发查询、遗留目录和外部占用场景完成 Windows 真机多轮复现与回归。
- [ ] 4.3 验证切换失败后旧 active 查询、UI 错误反馈和重启恢复。
  - **自测标准**：持续占用超过重试窗口时返回可定位错误，不伪报成功且不遗留不可判定目录状态。

## Phase 4：条件性性能优化与发布校验

- [ ] 5.1 仅针对结构化基线确认的真实瓶颈实施 provider 生命周期优化；若门槛已满足，记录“不需要优化”。
- [ ] 5.2 对每个实际优化使用同一夹具完成前后三轮比较，确认结果、权限、UI 响应、其它 scope 查询和回滚语义无回退。
