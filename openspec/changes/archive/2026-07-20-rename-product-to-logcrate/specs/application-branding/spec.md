...
系统 MUST 在品牌改名时保留已有 bundle identifier、用户配置与本地存储键、updater 签名信任和正式更新源，使已安装 LogPeek 的用户能够原位升级为 LogCrate。改名 MUST NOT 清空监控目录、筛选、语言、布局或其它持久化设置。

#### Scenario: 从旧品牌升级

- **WHEN** 已安装 LogPeek 的用户通过内置 updater 安装首个 LogCrate 版本
- **THEN** 系统将现有应用升级为 LogCrate，仅保留一个应用实例，并继续使用原监控配置和用户设置

#### Scenario: 新品牌继续检查更新

- **WHEN** LogCrate 启动自动检查或用户手动检查更新
- **THEN** 系统继续信任原 updater 公钥并从兼容的正式 Release endpoint 获取签名更新
