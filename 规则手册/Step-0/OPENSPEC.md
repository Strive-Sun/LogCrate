...
- 需求涉及 proposal、spec、新能力、公共行为变化、破坏性变更、架构调整或重大性能/安全工作时，继续读取 `openspec/AGENTS.md` 和相关 spec/change。
- 普通问答、只读调查以及 LogCrate 规则允许直接处理的简单修复或文档修改，无需继续加载 OpenSpec 资料，直接进入 Step-1。
...
`openspec/AGENTS.md` 可以增加具体要求，但不能静默取消 `Step-3/执行规则.md` 的任务顺序或 `Step-4/验证规则.md` 的验证与证据要求。无法按职责边界消解冲突时，停止实施并请求维护者裁决。
