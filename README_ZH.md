...
- **快速找到文件与关键行**：搜索本地文件名和路径，在日志内用 `Ctrl+F` 查找，或按识别出的时间、级别、离散值和文本字段筛选。
- **排查上下文不丢**：多文件选项卡分别保留滚动位置、编码和已加载内容。
- **重启继续排查**：恢复选项卡与字段布局，并在源文件变化或消失时安全处理。
...
| 日志查找与高亮 | `Ctrl+F` 支持正向/反向、全词、区分大小写、循环查找，并高亮可见匹配与当前命中    |
| 自适应字段筛选 | 识别常见日志布局，按时间范围、级别、离散值和文本组合筛选；字段布局可编辑、可恢复 |
| 本地文件搜索   | 可选建立本地文件名/路径索引，支持快速查询、日志预览及从结果加入监控             |
| 工作区恢复     | 恢复选项卡、活动文件、顺序、编码和字段布局，安全处理源文件变化或丢失             |
| 新日志通知     | 顶栏未读角标、逐条定位、一键全部已读                                             |
| 后缀筛选       | 自定义目录树与通知关注的文件后缀，也可临时显示全部文件                           |
| 签名自动更新   | 启动/手动检查、进度、签名验证与安装；Windows 优先 Pages 镜像，失败回退 GitHub    |
| 桌面体验       | 三套界面模板、深浅主题、单实例恢复、关闭到托盘、自动隐藏滚动条、可调目录栏宽度   |
...
- 在活动日志中按 `Ctrl+F`，可正向或反向查找、匹配全词、区分大小写，并选择是否循环。
- 对识别出的日志布局，可按分钟精度的时间范围、级别/离散值或文本内容组合筛选。
- 可选择“仅显示匹配”或“高亮匹配”，也可以决定是否保留未解析行。
- 可以拖动字段边界，或重命名、拆分、合并和修改字段类型；确认后的布局会为同一来源自动恢复。
...
在设置中启用 **“搜索本地文件”** 并重启 LogCrate，随后可通过顶栏“搜索”查询各本地卷中已索引的文件名和路径。双击日志结果可直接打开，也可以通过右键菜单把结果所在目录加入监控。

Windows 使用有界索引服务快速读取 MFT/USN 并增量恢复。搜索默认关闭，关闭时不会初始化索引运行时。
...
| 在当前日志中查找   | 按 `Ctrl+F`             | 定位并高亮匹配的关键词片段   |
| 按结构化字段筛选   | 使用正文上方的字段控件  | 按时间、级别、离散值或文本筛选/高亮 |
| 搜索本地文件名和路径 | 设置中启用搜索、重启后点击顶栏“搜索” | 查询本地元数据索引，不上传文件内容 |
...
- **日志布局**：可自动识别方括号应用日志、Chromium/CEF 与 Android logcat；其它格式回退为可编辑的正文单字段。
- **本地搜索**：可选持久化文件名/路径索引；Windows 使用 NTFS MFT/USN 快速发现，其它平台使用兼容 provider。
...
- 本地文件搜索为可选功能，修改后下次启动生效；文件内容始终留在本机，不会上传。
- Windows 自动更新优先从 Cloudflare Pages 获取签名 NSIS 包，失败时回退 GitHub；MSI 与 macOS 包仍由 GitHub 提供。
...
- [ ] **正则查找与结果总览**：在现有关键词导航上增加正则表达式、总数和紧凑结果列表。
...
- [ ] **跨文件时间线**：按时间关联选中的多份日志，同时保留每个文件的原始行和上下文。

### 长期：更多格式与更强工作流

- [ ] **结构化 JSON 日志视图**：针对 JSON Lines 展开字段、选择列，并提供超出现有文本布局筛选的可复用条件。
...
LogCrate 使用 Tauri 2 构建，前端负责交互与虚拟列表，Rust 后端负责文件系统监听、归档读取、编码检测、行/字段索引和本地文件搜索。

| 层                 | 职责                                               |
| ------------------ | -------------------------------------------------- |
| React + TypeScript | 目录树、通知、选项卡、设置、字段筛选、搜索界面和虚拟滚动正文 |
...
| SessionManager     | 编码检测、行/字段索引、窗口化筛选、有限会话 LRU 与临时资源清理 |
| SearchManager      | SQLite/Tantivy 持久元数据索引、多卷有界调度、查询与恢复 |
| Windows 索引服务  | 通过有界本地 IPC 执行高权限 MFT/USN 发现，并支持在设置中修复 |
...
│       ├── index.rs      # 行/字段索引、编码、筛选、缓存与会话生命周期
│       ├── search.rs     # 本地搜索调度、持久化、恢复与平台 provider
│       ├── search_index.rs # Tantivy 查询索引与快照切换
│       ├── ntfs/         # Windows MFT/USN 解析与有界服务 IPC
...
LogCrate 是采用 [GNU General Public License v3.0](LICENSE)（`GPL-3.0-only`）发布的自由软件。

随应用分发的第三方组件继续遵循各自的许可与署名要求，详见 [`resources/licenses/`](resources/licenses/)，其中包括 Orange 搜索模块的来源说明和 UnRAR 使用限制。
